...
from rankers.baselines import SearchResult, PairwiseLlmRanker, DuoT5LlmRanker
...
def main(args):
    ...
    #         ranker_class = MonoT5LlmRanker
    #     else:
    #         ranker_class = PointwiseLlmRanker
    ...
    for qid, query, ranking in tqdm(first_stage_rankings):
        ...
        total_consistent_comparison += ranker.total_consistent_comparison
    toc = time.time()
    ...
    print(f'Avg consistent comparison: {total_consistent_comparison/len(reranked_results)}')
    print(f'Avg new change: {ranker.new_change/len(reranked_results)}')
...
if __name__ == '__main__':
    ...
    run_parser.add_argument('--threshold', type=float, default=0.2)
    ...
    pairwise_parser.add_argument('--method', type=str, default='allpair',
                                 choices=['allpair', 'heapsort', 'bubblesort', 'double_elimination', 'double_elimination_reverse', 'swiss_system'])
